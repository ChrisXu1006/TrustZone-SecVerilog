...
// Declassification/Endorsement Load/Store
`define PISA_INST_DELW	  32'b11110?_?????_?????_?????_?????_??????
`define PISA_INST_DESW	  32'b11011?_?????_?????_?????_?????_??????
`define PISA_INST_DEDMA	  32'b1110??_?????_?????_?????_?????_??????
...
module pisa_InstTasks();
  ...
  function [`PISA_INST_NBITS-1:0] asm_fmt_de
  (
	input [`PISA_INST_OPCODE_NBITS-2:0] opcode,
	input								de_ns,
	input [`PISA_INST_RS_NBITS-1:0]		rs,
	input [`PISA_INST_RT_NBITS-1:0]		rt,
	input [`PISA_INST_IMM_NBITS-1:0]	imm
  ...
  begin
	asm_fmt_de[`PISA_INST_OPCODE] = {opcode,de_ns};
	asm_fmt_de[`PISA_INST_RS]	  = rs;
	asm_fmt_de[`PISA_INST_RT]	  = rt;
	asm_fmt_de[`PISA_INST_IMM]	  = imm;
  end
  endfunction  

  function [`PISA_INST_NBITS-1:0] asm_fmt_dedma
  (
	input [`PISA_INST_OPCODE_NBITS-3:0] opcode,
	input								dest_de_ns,
	input								src_de_ns,
	input [`PISA_INST_RS_NBITS-1:0]		rs,
	input [`PISA_INST_RT_NBITS-1:0]		rt,
  ...
  begin
	asm_fmt_dedma[`PISA_INST_OPCODE] = {opcode, dest_de_ns, src_de_ns};
	asm_fmt_dedma[`PISA_INST_RS]	  = rs;
	asm_fmt_dedma[`PISA_INST_RT]	  = rt;
	asm_fmt_dedma[`PISA_INST_IMM]	  = imm;
  end
  endfunction
  ...
  function [`PISA_INST_NBITS-1:0] asm_delw
  (
	input							 de_ns,
	input [`PISA_INST_RT_NBITS-1:0]	 rt,
	input [`PISA_INST_IMM_NBITS-1:0] imm,
  ...
  begin
	asm_delw = asm_fmt_de( 5'b11110, de_ns, rs, rt, imm );
  end
  endfunction

  function [`PISA_INST_NBITS-1:0] asm_desw
  (
	input							 de_ns,
	input [`PISA_INST_RT_NBITS-1:0]  rt,
	input [`PISA_INST_IMM_NBITS-1:0] imm,
  ...
  begin
	asm_desw = asm_fmt_de( 5'b11011, de_ns, rs, rt, imm );
  end
  endfunction

  function [`PISA_INST_NBITS-1:0] asm_dedma
  (
	input							 dest_ns,
	input							 src_ns,
	input [`PISA_INST_RT_NBITS-1:0]	 rt,
  ...
  begin
	asm_dedma = asm_fmt_dedma( 4'b1110, dest_ns, src_ns, rs, rt, imm );
  end
  endfunction
  ...
  begin
    ...
    case ( inst_str )
	  ...
	  "delw"	: begin e = $sscanf( str, "delw  %d, r%d, %s",ra,rb, roff_s );		asm = asm_delw  ( ra, rb,ro_s2o(roff_s),ro_s2r(roff_s));end 
	  "desw"    : begin e = $sscanf( str, "desw  %d, r%d, %s",ra,rb, roff_s );		asm = asm_desw	( ra, rb,ro_s2o(roff_s),ro_s2r(roff_s));end
	  "dedma"   : begin e = $sscanf( str, "dedma %d, %d, r%d, %s", ra, rb, rc, roff_s );asm = asm_dedma  ( ra, rb, rc, ro_s2o(roff_s),ro_s2r(roff_s));	end
